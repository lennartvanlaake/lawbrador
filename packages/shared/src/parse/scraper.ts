...
function parseNode(node: Node, $: CheerioAPI, index = 0): ParsedNode | null {
	...
	if ((!output.children || output.children?.length == 0) && !output.text?.trim()) return null;

	// flatten if this is a tag only containing one other tag
	if (output.children && output.children.length == 1 && !output.text && !output.children[0].text) {
