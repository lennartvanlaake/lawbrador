...
export const ALL_CONTAINER_TAGS = ALL_TAGS.filter(
  (it) => it.type == "container"
).map((it) => it.name);

export const ALL_PARAGRAPH_TAGS = ALL_TAGS.filter(
...
export const ALL_TAG_NAMES = ALL_TAGS.map((it) => it.name);
