import type { ParsedNode, SearchResult, SelectionRule, SourceSiteConfig } from '..';
import { hashUrlVariables, getFirstMatching, getAllMatching, makeLinkAbsolute } from '..';
const LINK_RULE: SelectionRule = { op: 'is', location: 'tag', value: 'a' };

export function parseSearchResults(root: ParsedNode, config: SourceSiteConfig): SearchResult[] {
	...
	const links = getAllMatching(base, LINK_RULE, config.htmlSearchRuleSet.resultLinkRule).map(
		(it) => {
			if (!it?.href) {
